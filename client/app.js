...
function renderAnTales(tales) {
  ...
  el.innerHTML = tales.map(function (t, i) {
    ...
    return '<div class="tale-card" onclick="openTaleAnalytics(\'' + esc(t.tale_id) + '\')" title="Открыть детальную аналитику">' +
      '<div class="tale-cover" style="background-image:url(\'' + anThumb(t.tale_id) + '\')"><span class="tale-rank">#' + (i + 1) + '</span></div>' +
...
async function anLoadTitles() {
  try {
    ...
    // Populate the "open any tale" picker, sorted by title.
    const sel = document.getElementById('an-tale-pick');
    if (sel) {
      const ids = Object.keys(anTitles).sort(function (a, b) { return anTitles[a].localeCompare(anTitles[b], 'ru'); });
      sel.innerHTML = '<option value="">— открыть аналитику любой сказки —</option>' +
        ids.map(function (id) { return '<option value="' + esc(id) + '">' + esc(anTitles[id]) + '</option>'; }).join('');
    }
  } catch (_) { /* ignore */ }
}
...
(function wireAnalytics() {
  ...
  // Per-tale modal: open from picker, close on ✕ / backdrop.
  if (q('an-tale-open')) q('an-tale-open').addEventListener('click', function () { openTaleAnalytics(q('an-tale-pick').value); });
